...
<div class="manage_all">
  <div class="user_content">
    ...
    <q-dialog v-model="popUp_edit" persistent>
  ...
  methods:{
    ...
    async addNewPoly(){
      ...
      if (this.saveBtn == 'Edit'){
        for(var p in this.currPoly){
          if(!(p in this.fields)) { this.fields[p] = this.currPoly[p]; }
        }
      }
      ...
      if (Object.values(this.problems).filter(v => v).length === 0) {
        if (this.saveBtn == 'Edit'){
          this.fields['id'] = this.currentId;
          for(var f in this.fields){ 
            this.currPoly[f] = this.fields[f];
          }
        }
        else this.fields['id'] = '';
