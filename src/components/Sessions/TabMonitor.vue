  ...
  <form id="monitor">
    <div style=" margin: 4em 6px 2rem 9px;">
      ...
      <template #body-cell-raw="props">
        <q-td>
        <a v-if="permits!='Sessions'" v-bind="this.download(props.row, 'edf')">EDF</a>
        </q-td>
      </template>
      ...
      <template #body-cell-results="props">
        <q-td>
          ...
          <a v-if="permits!='Sessions'" v-bind="this.download(props.row, 'json')">JSON</a>
        </q-td>
      </template>
...
export default {
  ...
  computed: {
    ...
    permits(){ return this.perms ? this.checkPermissions(): ''; },
  ...
  methods: {
    ...
    checkPermissions() {
      ...
      console.info('perms:' , this.perms);
      if(this.perms.includes('user-group') || this.perms.includes('EditSessions')|| this.perms.includes('ManageSessions')|| this.perms.includes('ManageDevices')) {
        return 'admin';
      } 
      else if(this.perms.includes('ManagePatients')) {
        return 'Patients';
      }
      else if(this.perms.includes('ViewSessions') || this.perms.includes('ManageSessions') || this.perms.includes('ViewHistory') || this.perms.includes('ViewLive')) {
        return 'Sessions';
      }
      else if(this.perms.includes('Gateway')) {
        return 'gateways';
